...
static int vidmode_init(x11_data *d) {
	int t1 = 0, t2 = 0;
...
static int vidmode_set_mode(x11_data *d, int width, int height) {
	Bool r;
	int mode;
...
static void vidmode_end(x11_data *d) {
	if (modecount > 0) {
...
static int x11_open(x11_data *d) {
...
static void x11_event_proc(XEvent *e) {
	KeySym sym;
	switch (e->type) {
		case KeyPress:
...
static int x11_display_init_proto(unsigned int w, unsigned int h, char *name, int argc, char** argv, int fullscreen) {
	...
	if (with_shm == 0)
		printf("WARNING: Cannot init MITSHM extension. Framerate will suck.\n");
